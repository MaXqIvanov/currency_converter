...
export const Header = () => {
  ...
  useEffect(() => {
      let current_nav = navigation_elems.filter((navigate: INavigation) => navigate.navigate.split('/')[1] === window.location.pathname.split('/')[1])[0].id
  ...
  return (
    <div className={'header'}>
      <div className='logo header__logo'><FcCurrencyExchange onClick={()=>{
        ...
        {navigation_elems.map((elem: INavigation) =>
          <div key={elem.id} onClick={()=>{
            ...
            navigate(elem.navigate)
             }} className={`current_navigate ${current_navigate === elem.id && 'current_navigate__active'}`}>
            {elem.title}
          </div>)}
      </div>
    </div>
