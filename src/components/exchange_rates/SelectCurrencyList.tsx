  ...
  return (
    <div>
      <div onClick={()=> setIsVisibleOptions(true)} className='custom_select'>
        <div className='currency_list__select' title='Курс по умолчанию'>{current_currency}</div>
        {is_visible_options &&
          <div ref={select_ref} className='currency_list__options'>
            {currency_list?.length > 0 && currency_list.map((item: string)=>
              <div className='currency_list__option' onClick={()=> dispatch(setCurrentCurrency({currency: item, navigate}))}>
                {item}
              </div>)}  
          </div>
          }
